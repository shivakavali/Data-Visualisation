...
df = pd.read_csv('C:/Temp/lang_data.csv')
...
df = pd.read_csv('C:/Temp/tips.csv')
st.dataframe(df)
...
fig = px.pie(df, values = 'total_bill', names = 'size', opacity=.9, color_discrete_sequence= px.colors.sequential.RdBu)
st.plotly_chart(fig)
